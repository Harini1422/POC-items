...
public class NewFile {
	...
	public static void main(String[] args) throws NoFilepatternException, GitAPIException, IOException {
	    ...
	    File file = new File(FOLDER_PATH +"/Project");
        if (!file.exists()) {
            if (file.mkdir()) {
                System.out.println("Directory is created!");
                File newfile = new File(FOLDER_PATH +"/Project/samplefile");
                newfile.createNewFile();
            }
        }
        File file1 = new File(FOLDER_PATH +"/Project1new");
        if (!file1.exists()) {
            if (file1.mkdir()) {
                System.out.println("Directory is created not commited!");
                File newfile = new File(FOLDER_PATH +"/Project1new/samplefile1");
                newfile.createNewFile();
            }
        }
        File file2 = new File(FOLDER_PATH +"/Project2new");
        if (!file2.exists()) {
            if (file2.mkdir()) {
                System.out.println("Directory is created not commited!");
                File newfile = new File(FOLDER_PATH +"/Project2new/samplefile2");
                newfile.createNewFile();
            }
        }
      //  result.reset().setMode( ResetType.MIXED ).call();
        result.add().addFilepattern("Project").call();    
        result.commit().setOnly("Project").setMessage("Added "+ "Project").call();
